...
BigUnsigned power(const BigUnsigned &base, const BigUnsigned &exp){
    if( exp == 0)
        return BigUnsigned(1);
    BigUnsigned ans = 1;
    for (BigUnsigned k=0; k <= exp; k++){
        ans *= base;
    }
    return ans;
}
...
BigUnsigned modexp_const(const BigInteger &base, const BigUnsigned &exponent,
				   const BigUnsigned &modulus) {
	BigUnsigned ans = 1, base2 = (base % modulus).getMagnitude();
	BigUnsigned::Index i = exponent.bitLength();
	// For each bit of the exponent, most to least significant...
	while (i > 0) {
		...
		ans %= modulus;
		// And multiply if the bit is a 1.
        ans *= power(base2, exponent.getBitAsInt(i));
        ans %= modulus;
	}
	return ans;
}